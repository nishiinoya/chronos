...
/**
 * Decide when a notification should fire for a given event.
 * Returns a Date or null.
 */
const DEFAULT_REMINDER_BEFORE_MIN = 10; // you can tweak

function getNotificationTimeForEvent(evt) {
  // For type "reminder" use reminderAt directly
  if (evt.type === "reminder" && evt.reminderAt) {
    return evt.reminderAt;
  }

  // For arrangements, send X minutes before the start
  if (evt.type === "arrangement" && evt.start) {
    return new Date(
      evt.start.getTime() - DEFAULT_REMINDER_BEFORE_MIN * 60 * 1000
    );
  }

  // For tasks, if dueDate exists, send at due date/time
  if (evt.type === "task" && evt.dueDate) {
    return evt.dueDate;
  }
...
export async function createEvent(req, res) {
  const userId = req.user.id;
  const { title, start, end, allDay, calendarId, location, description, type } =
    req.body || {};
  ...
  if (eventType === "arrangement") {
    if (!startDateRaw) {
      return res
        .status(400)
        .json({ error: "start is required for arrangement" });
    }
  ...
  await evt.save();
  // Create a notification for the event owner (you can extend to members later)
  const sendAt = getNotificationTimeForEvent(evt);
...
export async function updateEvent(req, res) {
  const userId = req.user.id;
  const { id } = req.params;
  const { title, start, end, allDay, location, description, type } =
    req.body || {};
  ...
  // Upsert a notification for the owner
  const sendAt = getNotificationTimeForEvent(evt);
  ...
  if (sendAt) {
    ...
    // If event no longer needs notification, remove pending ones
